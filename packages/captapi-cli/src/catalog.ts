...
const TIKTOK: Omit<Endpoint, "platform">[] = [
  ...
  { tool: "tiktok_profile_region", name: "TikTok Profile Region", path: "/v1/tiktok/profile-region", credits: 2, summary: "A creator's resolved country (region) plus interface language and core profile stats, from a profile URL or @handle. TikTok's own region is almost always null, so when missing region is AI-inferred from public bio/name/language cues; regionSource says whether it's \"tiktok\" or \"inferred\" and regionConfidence grades the guess.", params: [url(TT_PROFILE)] },
  { tool: "tiktok_audience_demographics", name: "TikTok Audience Demographics", path: "/v1/tiktok/audience-demographics", credits: 3, summary: "Ranked country breakdown of a TikTok creator's audience (audienceLocations: country, countryCode, count, percentage), sampled natively from the people commenting on their recent videos. Engagement-based signal, not a follower census. From a profile URL or @handle.", params: [url(TT_PROFILE)] },
  ...
  { tool: "tiktok_channel_posts", name: "TikTok Channel Posts", path: "/v1/tiktok/channel-posts", credits: 2, summary: "Latest videos from a TikTok profile — caption, engagement, thumbnail, sound, and hashtags for each post. Native cursor pagination (nextCursor).", params: [url(TT_PROFILE), limit(20, 200), { name: "cursor", type: "string", required: false, description: "Pagination cursor. Leave empty for the first page; then pass the nextCursor value returned in the previous response." }] },
  ...
  { tool: "tiktok_search_by_hashtag", name: "TikTok Search by Hashtag", path: "/v1/tiktok/search/hashtag", credits: 14, summary: "Search TikTok videos by hashtag — video URL, caption, author, and engagement counts per result, with cursor pagination (nextCursor + hasMore).", params: [q("Hashtag with or without the # (min 2 chars)."), limit(20, 100), { name: "cursor", type: "number", required: false, description: "Pagination offset. Leave at 0 for the first page; then pass the nextCursor value from the previous response. A null nextCursor means the end." }, { name: "region", type: "string", required: false, description: "Two-letter ISO country the scraping proxy is routed through. Default US. Only sets the proxy location — does not filter results by country." }] },
