...
const TIKTOK: Omit<Endpoint, "platform">[] = [
  ...
  { tool: "tiktok_profile_region", name: "TikTok Profile Region", path: "/v1/tiktok/profile-region", credits: 3, summary: "A creator's region (TikTok's own when exposed, else their dominant audience country — regionSource says which), an independent AI-estimated home country (estimatedRegion + confidence), interface language, and core profile stats, from a profile URL or @handle.", params: [url(TT_PROFILE)] },
