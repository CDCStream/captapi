...
> One API for structured data from ${PLATFORM_COUNT} social platforms — YouTube, TikTok, Instagram, Facebook, Twitter/X, Reddit, Threads, Bluesky, Pinterest, LinkedIn, Rumble, Twitch, Spotify, and more. Extract transcripts, AI summaries, comments, post/video details, profile/channel stats, search results, commerce data, and ad intelligence — with a single REST request that returns clean JSON.
...
5. For list/search/comment endpoints, start with a small \`limit\` unless the user asks for more. Pass cache=true for a free 24h cache hit (default is always fresh). Many native list/search/comment calls are flat 2 credits (YouTube comments/search/channel-videos/playlists, TikTok comments/channel-posts/top-search/trending-feed, Instagram hashtag-search, Twitter/X user-tweets/search, Reddit list/search/comments). Other list endpoints may still scale by result count — check each endpoint's Cost line below.
