...
import { AtSign, ExternalLink, Eye, Loader2, ShieldCheck, Sparkles } from "lucide-react";
...
export default function InstagramHighlightsViewerClient() {
  ...
  function handleView() {
    ...
    window.setTimeout(() => {
      setUsername(handle);
      setLoading(false);
      toast.success("Profile ready. Open the highlights below.");
    }, 450);
  }
        ...
        {username ? (
          <div className="mt-6 rounded-xl border border-white/10 bg-zinc-900 p-4 sm:p-5">
            ...
            <div className="mt-4">
              <p className="flex items-center gap-2 text-sm font-medium text-white">
                <Sparkles className="size-4 text-primary" /> View this profile&apos;s highlights
              </p>
              <ol className="mt-3 space-y-2 text-sm text-gray-300">
                <li>1. Open the public profile using the button below.</li>
                <li>2. The highlight covers appear in a row under the bio &mdash; tap any cover to play it.</li>
                <li>3. Instagram does not notify the owner when you view their permanent highlights.</li>
              </ol>
              ...
              <p className="mt-3 flex items-start gap-2 text-xs text-gray-500">
                ...
                Only public highlights can be viewed. Respect Instagram&apos;s Terms of Service and other people&apos;s privacy.
              </p>
            </div>
          </div>
        ) : (
          <div className="mt-6 flex min-h-[180px] flex-col items-center justify-center rounded-xl border border-dashed border-white/15 bg-zinc-900 p-6 text-center">
            ...
            <p className="mt-1 max-w-sm text-sm text-gray-400">
              We will prepare a quick, anonymous way to open and browse a public profile&apos;s story highlights.
            </p>
          </div>
        )}
      </div>
