...
def _transcript_error_payload(
    *,
    ...
    """Empty-segment diagnostic for the transcript router (no STT — captions only)."""
    return {
        "segments": [],
        "title": title,
...
async def transcript_native(norm_url: str, language: str | None) -> dict[str, Any] | None:
    ...
    Returns captions YouTube published — never speech-to-text. When the player
    is reachable but has zero tracks, returns a diagnostic payload
    (``segments=[]`` + ``error.code``) so the router can 404 without guessing.
    When ``language`` is set, never silently returns a different language's
    track; either match/translate into that language or return
    ...
    has_auto = any(t.get("kind") == "asr" for t in tracks)
    requested = safe_str(language)

    if not tracks:
        return _transcript_error_payload(
            code="no_captions",
            reason=(
    ...
    want = requested.lower()[:2] if requested else None

    async def _try_track(
        track: dict[str, Any], *, use_tlang: bool
    ) -> dict[str, Any] | None:
        base_url = safe_str(track.get("baseUrl"))
        if not base_url:
            return None
        url = (
            f"{base_url}&tlang={requested}"
            if use_tlang and requested
            else base_url
        )
        ...
        if not segments:
            return None
        track_code = safe_str(track.get("languageCode"))
        returned = requested if use_tlang and requested else track_code
        return {
            "segments": segments,
            "title": title,
            "language": returned,
    ...
    if want:
        matching = [
            t
            for t in tracks
            if (t.get("languageCode") or "").lower().startswith(want)
        ]
        # Prefer manual over ASR for the requested language.
        matching_sorted = sorted(
            matching, key=lambda t: 0 if t.get("kind") != "asr" else 1
        )
        for track in matching_sorted:
            got = await _try_track(track, use_tlang=False)
            if got:
                return got
        # YouTube timedtext `tlang=` can translate another track into the
        # requested language — label as translated; never return a raw other
        # language when the client asked for a specific one.
        for track in _rank_caption_tracks(tracks, None):
            code = (track.get("languageCode") or "").lower()
            if code.startswith(want):
                continue
            got = await _try_track(track, use_tlang=True)
            if got:
                return got
        return _transcript_error_payload(
            code="language_not_available",
            reason=(
                f"No caption track matches language '{requested}', and "
                "YouTube did not return a translation into that language."
            ),
            title=title,
            available=available,
            has_auto=has_auto,
    ...
    # No language preference — pick the best available track (en / manual first).
    for track in _rank_caption_tracks(tracks, None):
        got = await _try_track(track, use_tlang=False)
        if got:
            return got
    ...
    return _transcript_error_payload(
        code="no_captions",
        reason=(
            "Caption tracks were listed but no timedtext payload could be fetched."
        ),
        title=title,
        available=available,
        has_auto=has_auto,
